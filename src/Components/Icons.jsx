...
function Icons(props) {
  ...
  return (
    <div className='flex items-center gap-3'>
            {Logo.map((item,index)=><a href="./Navbar.jsx" className={`h-10 w-10 rounded-full ${props.background==="white"? "bg-white":"bg-[#9b9b9b]"} text-black flex justify-center items-center`} key={index}>{item}</a>)}
        </div>
  )
}
